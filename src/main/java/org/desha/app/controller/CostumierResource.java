...
public class CostumierResource {
    ...
    @GET
    @Path("{id}")
    public Uni<Costumier> getCostumierById(Long id) {
        return costumierService.getById(id);
    }

    @GET
    @Path("all")
    public Uni<Response> getAllCostumiers() {
        return
    ...
    public Uni<Response> getCostumiers(
        ...
        return
                costumierService.get(pageIndex, size, sort, sortDirection, term, countryIds, fromBirthDate, toBirthDate, fromDeathDate, toDeathDate, fromCreationDate, toCreationDate, fromLastUpdate, toLastUpdate)
                        .flatMap(personDTOList ->
                                costumierService.count(term, countryIds, fromBirthDate, toBirthDate, fromDeathDate, toDeathDate, fromCreationDate, toCreationDate, fromLastUpdate, toLastUpdate).map(total ->
    ...
    public Uni<Response> getMovies(
        ...
        return
                costumierService.getMovies(id, page, size, sort, sortDirection, term)
                        .flatMap(movieList ->
                                costumierService.countMovies(id, term).map(total ->
                                        movieList.isEmpty()
                                                ? Response.noContent().header(CustomHttpHeaders.X_TOTAL_COUNT, total).build()
    ...
    public Uni<Response> getPhoto(@PathParam("fileName") String fileName) {
        ...
        return
                costumierService.getPhoto(fileName)
