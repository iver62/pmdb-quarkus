...
public class MusicianResource {
    ...
    @GET
    @Path("{id}")
    public Uni<Musician> getMusicianById(Long id) {
        return musicianService.getById(id);
    }

    @GET
    @Path("all")
    public Uni<Response> getAllMusicians() {
        return
    ...
    public Uni<Response> getMusicians(
        ...
        return
                musicianService.get(pageIndex, size, sort, sortDirection, term, countryIds, fromBirthDate, toBirthDate, fromDeathDate, toDeathDate, fromCreationDate, toCreationDate, fromLastUpdate, toLastUpdate)
                        .flatMap(personDTOList ->
                                musicianService.count(term, countryIds, fromBirthDate, toBirthDate, fromDeathDate, toDeathDate, fromCreationDate, toCreationDate, fromLastUpdate, toLastUpdate).map(total ->
    ...
    public Uni<Response> getMovies(
        ...
        return
                musicianService.getMovies(id, page, size, sort, sortDirection, term)
                        .flatMap(movieList ->
                                musicianService.countMovies(id, term).map(total ->
                                        movieList.isEmpty()
                                                ? Response.noContent().header(CustomHttpHeaders.X_TOTAL_COUNT, total).build()
    ...
    public Uni<Response> getPhoto(@PathParam("fileName") String fileName) {
        ...
        return
                musicianService.getPhoto(fileName)
