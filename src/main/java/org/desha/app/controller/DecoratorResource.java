...
public class DecoratorResource {
    ...
    @GET
    @Path("{id}")
    public Uni<Decorator> getDecoratorById(Long id) {
        return decoratorService.getById(id);
    }

    @GET
    @Path("all")
    public Uni<Response> getAllDecorators() {
        return
    ...
    public Uni<Response> getDecorators(
        ...
        return
                decoratorService.get(pageIndex, size, sort, sortDirection, term, countryIds, fromBirthDate, toBirthDate, fromDeathDate, toDeathDate, fromCreationDate, toCreationDate, fromLastUpdate, toLastUpdate)
                        .flatMap(personDTOList ->
                                decoratorService.count(term, countryIds, fromBirthDate, toBirthDate, fromDeathDate, toDeathDate, fromCreationDate, toCreationDate, fromLastUpdate, toLastUpdate).map(total ->
    ...
    public Uni<Response> getMovies(
        ...
        return
                decoratorService.getMovies(id, page, size, sort, sortDirection, term)
                        .flatMap(movieList ->
                                decoratorService.countMovies(id, term).map(total ->
                                        movieList.isEmpty()
                                                ? Response.noContent().header(CustomHttpHeaders.X_TOTAL_COUNT, total).build()
    ...
    public Uni<Response> getPhoto(@PathParam("fileName") String fileName) {
        ...
        return
                decoratorService.getPhoto(fileName)
