...
public class ProducerResource {
    ...
    @GET
    @Path("{id}")
    public Uni<Producer> getProducerById(Long id) {
        return producerService.getById(id);
    }

    @GET
    @Path("all")
    public Uni<Response> getAllProducers() {
        return
    ...
    public Uni<Response> getProducers(
        ...
        return
                producerService.get(pageIndex, size, sort, sortDirection, term, countryIds, fromBirthDate, toBirthDate, fromDeathDate, toDeathDate, fromCreationDate, toCreationDate, fromLastUpdate, toLastUpdate)
                        .flatMap(personDTOList ->
                                producerService.count(term, countryIds, fromBirthDate, toBirthDate, fromDeathDate, toDeathDate, fromCreationDate, toCreationDate, fromLastUpdate, toLastUpdate).map(total ->
    ...
    public Uni<Response> getMovies(
        ...
        return
                producerService.getMovies(id, page, size, sort, sortDirection, term)
                        .flatMap(movieList ->
                                producerService.countMovies(id, term).map(total ->
                                        movieList.isEmpty()
                                                ? Response.noContent().header(CustomHttpHeaders.X_TOTAL_COUNT, total).build()
    ...
    public Uni<Response> getPhoto(@PathParam("fileName") String fileName) {
        ...
        return
                producerService.getPhoto(fileName)
