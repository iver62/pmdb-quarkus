...
public class ActorResource {
    ...
    @GET
    @Path("{id}")
    public Uni<Actor> getActorById(Long id) {
        return actorService.getById(id);
    }

    @GET
    @Path("all")
    public Uni<Response> getAllActors() {
        return
                actorService.getAll()
    ...
    public Uni<Response> getActors(
        ...
        return
                actorService.get(pageIndex, size, sort, sortDirection, term, countryIds, fromBirthDate, toBirthDate, fromDeathDate, toDeathDate, fromCreationDate, toCreationDate, fromLastUpdate, toLastUpdate)
                        .flatMap(personDTOList ->
                                actorService.count(term, countryIds, fromBirthDate, toBirthDate, fromDeathDate, toDeathDate, fromCreationDate, toCreationDate, fromLastUpdate, toLastUpdate).map(total ->
    ...
    public Uni<Response> getMovies(
        ...
        return
                actorService.getMovies(id, page, size, sort, sortDirection, term)
                        .flatMap(movieList ->
                                actorService.countMovies(id, term).map(total ->
                                        movieList.isEmpty()
                                                ? Response.noContent().header(CustomHttpHeaders.X_TOTAL_COUNT, total).build()
    ...
    public Uni<Response> getPhoto(@PathParam("fileName") String fileName) {
        ...
        return
                actorService.getPhoto(fileName)
    ...
    public Uni<Response> save(PersonDTO personDTO) {
        return
                actorService
                        .save(personDTO)
                        .onItem().ifNotNull()
                        .transform(actor -> Response.ok(actor).status(CREATED).build())
                ;
    }
    ...
    public Uni<Response> update(
        ...
        return
                actorService
                        ...
                        .onItem().ifNotNull().transform(actor -> Response.ok(actor).build())
