...
public class DirectorResource {
    ...
    @GET
    @Path("{id}")
    public Uni<Director> getDirectorById(Long id) {
        return directorService.getById(id);
    }

    @GET
    @Path("all")
    public Uni<Response> getAllDirectors() {
        return
    ...
    public Uni<Response> getDirectors(
        ...
        return
                directorService.get(pageIndex, size, sort, sortDirection, term, countryIds, fromBirthDate, toBirthDate, fromDeathDate, toDeathDate, fromCreationDate, toCreationDate, fromLastUpdate, toLastUpdate)
                        .flatMap(personDTOList ->
                                directorService.count(term, countryIds, fromBirthDate, toBirthDate, fromDeathDate, toDeathDate, fromCreationDate, toCreationDate, fromLastUpdate, toLastUpdate).map(total ->
    ...
    public Uni<Response> getMovies(
        ...
        return
                directorService.getMovies(id, page, size, sort, sortDirection, term)
                        .flatMap(movieList ->
                                directorService.countMovies(id, term).map(total ->
                                        movieList.isEmpty()
                                                ? Response.noContent().header(CustomHttpHeaders.X_TOTAL_COUNT, total).build()
    ...
    public Uni<Response> getPhoto(@PathParam("fileName") String fileName) {
        ...
        return
                directorService.getPhoto(fileName)
    ...
    public Uni<Response> save(PersonDTO personDTO) {
        return
                directorService
                        .save(personDTO)
                        .onItem().ifNotNull()
                        .transform(director -> Response.ok(director).status(CREATED).build())
                ;
    }
