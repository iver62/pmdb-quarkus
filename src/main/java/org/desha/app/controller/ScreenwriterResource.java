...
public class ScreenwriterResource {
    ...
    @GET
    @Path("{id}")
    public Uni<Screenwriter> getScreenwriterById(Long id) {
        return screenwriterService.getById(id);
    }

    @GET
    @Path("all")
    public Uni<Response> getAllScreenwriters() {
        return
    ...
    public Uni<Response> getScreenwriters(
        ...
        return
                screenwriterService.get(pageIndex, size, sort, sortDirection, term, countryIds, fromBirthDate, toBirthDate, fromDeathDate, toDeathDate, fromCreationDate, toCreationDate, fromLastUpdate, toLastUpdate)
                        .flatMap(personDTOList ->
                                screenwriterService.count(term, countryIds, fromBirthDate, toBirthDate, fromDeathDate, toDeathDate, fromCreationDate, toCreationDate, fromLastUpdate, toLastUpdate).map(total ->
    ...
    public Uni<Response> getMovies(
        ...
        return
                screenwriterService.getMovies(id, page, size, sort, sortDirection, term)
                        .flatMap(movieList ->
                                screenwriterService.countMovies(id, term).map(total ->
                                        movieList.isEmpty()
                                                ? Response.noContent().header(CustomHttpHeaders.X_TOTAL_COUNT, total).build()
    ...
    public Uni<Response> getPhoto(@PathParam("fileName") String fileName) {
        ...
        return
                screenwriterService.getPhoto(fileName)
